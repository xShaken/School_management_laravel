...
<div class="container-div">
    ...
    <div class="st">
        ...
        <table class="table">
            <thead>
                <tr>
                    <th style="width: 20%;" scope="col">Teacher ID</th>
            ...
            <tbody>
                ...
                <tr>
                    <td style="width: 20%;">{{ $teacher->Teacher_id }}</td>
                    <td style="width: 20%;">{{ $teacher->Name }}</td>
                    <td style="width: 20%;">{{ $teacher->Email }}</td>
                    <td style="width: 20%;">{{ $teacher->Address }}</td>
                    <td style="display:flex; gap:1rem;">
                        <a href="{{ route('teachers.show', ['teacher' => $teacher]) }}" class="btn btn-info btn-sm">Show</a>
                        <a href="{{ route('teachers.edit', ['teacher' => $teacher]) }}" class="btn btn-secondary btn-sm">Edit</a>
                        <form method="POST" action="{{ route('teachers.destroy', ['teacher' => $teacher]) }}" style="display:inline;">
                            @csrf
                            @method('DELETE')
